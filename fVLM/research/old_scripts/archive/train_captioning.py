"""
Train video captioning using streaming WebVid data.

Uses the forward_captioning method to train the model to generate
captions from video features using cross-entropy loss.
"""
...
class CaptioningDataset(IterableDataset):
    """Streaming dataset for video captioning training."""
...
def main():
    config = {
        ...
        'weight_decay': 0.01,
        'warmup_steps': 500,
        'max_steps': 25000,  # ~8 hours at ~1s/step
        'grad_clip': 1.0,
        'log_every': 100,
        'save_every': 2500,
        'eval_every': 1000,
    ...
    if HAS_WANDB:
        wandb.init(
            ...
            config=config,
            name=f"captioning_{datetime.now().strftime('%m%d_%H%M')}",
        )
    ...
    # Optimizer - lower LR for LLM, normal for new params
    optimizer = torch.optim.AdamW([
        {'params': model.encoder.parameters(), 'lr': config['learning_rate']},
        {'params': model.llm.parameters(), 'lr': config['learning_rate'] * 0.1},  # Lower for LLM
        {'params': [model.video_end_token], 'lr': config['learning_rate']},
        {'params': model.dino_to_llm.parameters(), 'lr': config['learning_rate']},
    ...
    # Dataset
    print("\nCreating dataset...")
    base_dataset = CaptioningDataset(
    ...
    while global_step < config['max_steps']:
        for batch in dataloader:
            ...
            if global_step % config['log_every'] == 0:
                ...
                pbar.set_postfix({'loss': f'{avg_loss:.4f}', 'ppl': f'{np.exp(avg_loss):.1f}', 'success%': f'{success_rate:.0f}'})
            ...
            if global_step % config['eval_every'] == 0:
                model.eval()
                with torch.no_grad():
                    sample_frames = batch['frames'][:1].to(device)
                    with torch.autocast(device_type='cuda', dtype=torch.bfloat16):
