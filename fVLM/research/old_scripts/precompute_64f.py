#!/usr/bin/env python3
"""
Precompute 64-frame data for full comparison experiments.

Target: 5000+ samples (enough for 300 steps × 16 batch = 4800 samples < 1 epoch)

Output format (matching existing 64F shards):
- frames: [64, 3, 224, 224] as uint8
- latents: [64, 4, 28, 28] as bfloat16
- caption: str
"""
...
CONFIG = {
    "output_dir": "/mnt/d/projects/fVLM/data/webvid_64f_5k",
    "num_frames": 64,
    "frame_size": 224,
    "min_duration": 15,  # Need longer videos for 64 frames
    "max_duration": 120,
    "prefetch_threads": 8,
...
def extract_frames_fast(video_bytes: bytes, num_frames: int, size: int) -> torch.Tensor:
    ...
    try:
        with tempfile.TemporaryDirectory() as tmpdir:
            # Extract at higher fps for 64 frames
            cmd = [
                'ffmpeg', '-i', temp_path,
                '-vf', f'scale={size}:{size}',
                '-r', '16',  # 16 fps for longer videos
                '-q:v', '2',
...
class VAEEncoder:
    ...
    def encode(self, frames: torch.Tensor) -> torch.Tensor:
        ...
        for i in range(0, len(frames), batch_size):
            ...
            x = batch.to(self.device, dtype=torch.bfloat16) / 255.0
...
def main():
    ...
    print("=" * 70)
    print("PRECOMPUTE 64-FRAME DATA")
    print("=" * 70)
